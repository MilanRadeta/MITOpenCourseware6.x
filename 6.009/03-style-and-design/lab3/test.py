...
def test_tiny_bacon_path():
    ...
    assert lab.bacon_path(db_tiny, actor_id) == expected
...
def test_bacon_path_02():
    ...
    check_valid_path(raw_db_small, first_result, 4724, actor_id, len_expected)
    check_valid_path(raw_db_small, second_result, 4724, actor_id, len_expected)
...
def test_bacon_path_03():
    ...
    check_valid_path(raw_db_large, result, 4724, actor_id, len_expected)
...
def test_actor_to_actor_path_01():
    ...
    check_valid_path(raw_db_large, first_result, actor_1, actor_2, len_expected)
    check_valid_path(raw_db_large, second_result, actor_1, actor_2, len_expected)
...
def check_valid_path(d, p, s, e, l):
    lp = len(p)-1 if p is not None else -1
    assert lp == l
    assert s is None or p[0] == s
    assert e is None or p[-1] == e
    x = {frozenset(i[:-1]) for i in d}
    assert all(frozenset(i) in x for i in zip(p, p[1:]))
...
def check_connected_movie_path(m1, m2, expected_length):
    ...
    check_valid_path(raw_db_large, result, None, None, expected_length)
