...
def websisOut(students):
    f = open(os.path.join(ROOT_DIRECTORY, "websis.txt"), "w")
    f.write("     FALL TERM 2020-2021      Registration Class List      6.666  U       17-NOV-20\n")
    f.write("\n")
...
def gradesXmlOut(students):
    f = open(os.path.join(ROOT_DIRECTORY, "grades.xml"), "w")
    f.write("<grades>\n")
...
def csvSectionOut(students, plural, singular, howMany):
    for item in range(1, howMany):
        f = open(os.path.join(ROOT_DIRECTORY, "grades", singular + str(item) + ".csv"), "w")
...
def gradesCsvOut(students):
    os.mkdir(os.path.join(ROOT_DIRECTORY, "grades"))
